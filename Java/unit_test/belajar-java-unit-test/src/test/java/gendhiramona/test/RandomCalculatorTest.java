...
public class RandomCalculatorTest extends AbstarctCalculatorTest {
    ...
    @DisplayName("RandomRepeat")
    @RepeatedTest(
            value = 10,
            name = "{displayName} for {currentRepetition} of {totalRepetitions}"
    )
    ...
    @DisplayName("RandomRepeatInfo")
    @RepeatedTest(
            value = 10,
            name = "{displayName} for {currentRepetition} of {totalRepetitions}"
    )
    void testRandomRepeatInfo(TestInfo info, Random random, RepetitionInfo repetitionInfo) {
        System.out.println(info.getDisplayName() + " for " + repetitionInfo.getCurrentRepetition() + " of " + repetitionInfo.getTotalRepetitions());
        var a = random.nextInt();
        var b = random.nextInt();
    ...
    @ParameterizedTest(name = "{displayName} of {0}")
    ...
    public static List<Integer> parameterSource(){
        return List.of(1, 2, 3, 20, 5, 6, 7, 8, 9);
    }
