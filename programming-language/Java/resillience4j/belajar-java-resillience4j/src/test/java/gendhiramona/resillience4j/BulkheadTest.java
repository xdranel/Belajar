...
//        log.info(String.valueOf(Runtime.getRuntime().availableProcessors()));
        ...
        for (int i = 0; i < 1000; i++) {
            Supplier<CompletionStage<Void>> supplier =
                    ThreadPoolBulkhead.decorateRunnable(bulkhead, () -> slow());
            supplier.get();
        }
...
//                .queueCapacity(1)
    ...
    void testSemaphoreRegistry() throws InterruptedException {
        ...
        Bulkhead bulkhead = registry.bulkhead("test", "config");
    ...
    void testThreadPoolRegistry() throws InterruptedException {
        ...
        ThreadPoolBulkhead bulkhead = registry.bulkhead("test", "config");
