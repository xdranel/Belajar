...
print(
    f"Number_of_Founders - Min: {df['Number_of_Founders'].min()}, Max: {df['Number_of_Founders'].max()}"
)
print(
    f"Employees_Count - Min: {df['Employees_Count'].min()}, Max: {df['Employees_Count'].max()}"
)
print(
    f"Funding_Amount - Min: {df['Funding_Amount'].min()}, Max: {df['Funding_Amount'].max()}"
)
...
founders_min, founders_max = (
    df["Number_of_Founders"].min(),
    df["Number_of_Founders"].max(),
)
...
startup_age = ctrl.Antecedent(
    np.arange(max(0, age_min - 1), age_max + 2, 1), "Startup_Age"
)
number_of_founders = ctrl.Antecedent(
    np.arange(max(1, founders_min), founders_max + 1, 1), "Number_of_Founders"
)
employees_count = ctrl.Antecedent(
    np.arange(max(1, employees_min), employees_max + 1, 1), "Employees_Count"
)
funding_amount = ctrl.Antecedent(
    np.arange(max(10000, funding_min), funding_max + 1, 1000), "Funding_Amount"
)
...
number_of_founders["low"] = fuzz.trimf(
    number_of_founders.universe, [founders_min, founders_min, founders_mid]
)
number_of_founders["medium"] = fuzz.trimf(
    number_of_founders.universe, [founders_min, founders_mid, founders_max]
)
number_of_founders["high"] = fuzz.trimf(
    number_of_founders.universe, [founders_mid, founders_max, founders_max]
)

emp_third = (employees_max - employees_min) / 3
employees_count["low"] = fuzz.trimf(
    employees_count.universe, [employees_min, employees_min, employees_min + emp_third]
)
employees_count["medium"] = fuzz.trimf(
    employees_count.universe,
    [employees_min, employees_min + emp_third, employees_min + 2 * emp_third],
)
employees_count["high"] = fuzz.trimf(
    employees_count.universe, [employees_min + emp_third, employees_max, employees_max]
)

fund_third = (funding_max - funding_min) / 3
funding_amount["low"] = fuzz.trimf(
    funding_amount.universe, [funding_min, funding_min, funding_min + fund_third]
)
funding_amount["medium"] = fuzz.trimf(
    funding_amount.universe,
    [funding_min, funding_min + fund_third, funding_min + 2 * fund_third],
)
funding_amount["high"] = fuzz.trimf(
    funding_amount.universe, [funding_min + fund_third, funding_max, funding_max]
)
